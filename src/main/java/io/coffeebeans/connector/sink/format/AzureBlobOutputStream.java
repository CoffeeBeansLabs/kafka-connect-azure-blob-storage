...
public class AzureBlobOutputStream extends PositionOutputStream {
     ...
     * {@link io.coffeebeans.connector.sink.format.json.JsonRecordWriter JsonRecordWriter} and
     * {@link io.coffeebeans.connector.sink.format.bytearray.ByteArrayRecordWriter ByteArrayRecordWriter}
    ...
    private void stageBlock(final int partSize, boolean shouldCommit) throws IOException {
        try {
            ...
            this.storageManager.commitBlockIdsAsync(blobName, base64BlockIds, false)
                    .subscribe(
                            success -> log.debug("Commit successful for blob: {}", blobName),
                            error -> log.debug("Commit failed for blob: {}", blobName));
